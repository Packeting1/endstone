...
bool EndstonePlayer::handlePacket(Packet &packet)
{
    ...
    case MinecraftPacketIds::PlayerAuthInputPacket: {
        ...
        if (pk.getInput(PlayerAuthInputPacket::StartSpinAttack)) {
        ...
        for (auto it = actions.begin(); it != actions.end();) {
            ...
            std::optional<BlockPos> damage_position;
            ...
            if (damage_position) {
                block = getDimension().getBlockAt(damage_position->x, damage_position->y, damage_position->z);
                position = Vector{damage_position->x, damage_position->y, damage_position->z};
            }
...
void EndstonePlayer::handlePacketPost(Packet &packet)
{
    if (packet.getId() != MinecraftPacketIds::BookEdit || pending_book_meta_ == nullptr) {
        return;
    }
