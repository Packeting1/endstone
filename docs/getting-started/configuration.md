...
- Global anticheat, block-update, chunk-loading, chunk-system, command, message settings other than the active `messages.no-permission` and `use-display-name-in-quit-message`, player-auto-save, proxy, scoreboard, spam-limiter settings other than the active `incoming-packet-threshold`, Spark, time, update-checker, watchdog, and other `misc` settings except the active `max-joins-per-tick` and `enable-nether` entries.
