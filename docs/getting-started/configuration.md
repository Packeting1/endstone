...
The current source tree wires the empty-world Level tick setting, packet limiter, join throttling, Nether toggle, book validation, void damage, and the two additional world collision entries. Other template values remain synchronized but are not runtime consumers in this checkout.
...
- World anticheat/anti-xray, chunk save and unload, world collision settings other than the two active collision entries above, command-block, entity behavior, entity spawning and tracking, environment settings other than `disable-thunder`, `disable-explosion-knockback`, and `void-damage-amount`, feature-seed, fishing, fix, hopper, lootable, map, growth, world-misc, scoreboard, spawn, tick-rate, and unsupported-settings other than the active world-ticking entry.
