...
| `paper.global.messages.no-permission` | `messages.no-permission` | Replaces the generic permission-denied translation used by Endstone command senders. The configured string is returned for console, block, origin, and player error-message paths. |
| `paper.global.messages.use-display-name-in-quit-message` | `messages.use-display-name-in-quit-message` | When `true`, the existing PlayerDisconnectEvent path uses a non-empty Bedrock player name tag as the quit-message name; otherwise it uses the account name. |
...
| `paper.global.packet-limiter.all-packets.*` / `paper.global.packet-limiter.overrides.*` | `packet-limiter.all-packets.*`, `packet-limiter.overrides.*` | The packet dispatcher counts incoming packets per network address over the configured interval. A matching per-packet override, keyed by `Packet::getName()`, replaces the all-packets interval, rate, and action. Over-limit packets are dropped for `DROP`; `KICK` disconnects the sender with `packet-limiter.kick-message`. |
| `paper.global.spam-limiter.incoming-packet-threshold` | `spam-limiter.incoming-packet-threshold` | A non-negative millisecond threshold admits the first eight packets in a per-connection window and drops subsequent packets until the window expires. `disabled` or a negative value leaves this guard inactive. |
| `paper.global.spam-limiter.recipe-spam-increment` / `recipe-spam-limit` | `spam-limiter.recipe-spam-increment`, `spam-limiter.recipe-spam-limit` | Recipe-book craft actions use a per-player tick-decaying counter. Each recipe action adds the configured increment; reaching the positive limit kicks the player with `disconnect.spam`. Non-positive increment or limit disables this guard. |
...
| `paper.world_defaults.maps.item-frame-cursor-limit` | `maps.item-frame-cursor-limit` | Limits visible map cursors serialized into client map-data packets. A negative value disables the cap. |
| `paper.world_defaults.maps.item-frame-cursor-update-interval` | `maps.item-frame-cursor-update-interval` | Updates cached frame cursors at the configured interval in ticks; non-positive values suppress frame cursor updates. |
...
| `paper.world_defaults.environment.void-damage-min-build-height-offset` | `environment.void-damage-min-build-height-offset` | Suppresses native Void hurt events at or above the configured offset from the dimension minimum build height. The Nether ceiling height can opt an event back in when configured. |
| `paper.world_defaults.environment.nether-ceiling-void-damage-height` | `environment.nether-ceiling-void-damage-height` | A numeric Nether height allows existing native Void hurt events at or above that ceiling; `disabled` keeps ceiling damage suppressed. |
...
- Global anticheat, block-update, chunk-loading, chunk-system, command, message settings other than the active `messages.no-permission` and `use-display-name-in-quit-message`, player-auto-save, proxy, scoreboard, spam-limiter settings other than the active `incoming-packet-threshold`, `recipe-spam-increment`, and `recipe-spam-limit`, Spark, time, update-checker, watchdog, and other `misc` settings except the active `max-joins-per-tick` and `enable-nether` entries.
...
- World anticheat/anti-xray, chunk save and unload settings other than the active unloaded-chunk movement guard, world collision settings other than the three active collision entries above, command-block, entity behavior other than the active mob-effect settings, entity spawning and tracking other than the active monster-light setting, environment settings other than `disable-thunder`, `disable-explosion-knockback`, `void-damage-amount`, `void-damage-min-build-height-offset`, and `nether-ceiling-void-damage-height`, feature-seed, fishing, fix, hopper, lootable, map settings other than the active `item-frame-cursor-limit` and `item-frame-cursor-update-interval`, growth, world-misc, scoreboard, spawn, tick-rate, and unsupported-settings other than the active world-ticking entry.
