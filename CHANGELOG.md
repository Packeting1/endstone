...
- Added runtime consumers for Paper configuration: book page, author, title, and total-size validation; permission-denied messages; configurable quit-message names; recipe-book spam throttling; packet-rate limiting, per-packet overrides, and incoming-spam limiting; authenticated join throttling; Nether transitions; empty-world ticking; unloaded-chunk movement guards; collision filtering and per-tick collision caps; mob-effect immunity; monster spawn light limits; configurable Void damage thresholds and amount; map cursor limits and update intervals; and the existing weather and explosion settings.
